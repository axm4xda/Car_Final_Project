    ...
    public class CarFeatureService : ICarFeatureService
    {
        ...
        public async Task UpdateAsync(CarFeature feature)
        {
            ...
            var existing = await _context.CarFeatures.FindAsync(feature.Id)
                ?? throw new KeyNotFoundException($"Id={feature.Id} olan xüsusiyy?t tap?lmad?.");
        ...
        public async Task DeleteAsync(int id)
        {
            var feature = await _context.CarFeatures
                .Include(cf => cf.Cars)
                .FirstOrDefaultAsync(cf => cf.Id == id)
                ?? throw new KeyNotFoundException($"Id={id} olan xüsusiyy?t tap?lmad?.");

            if (feature.Cars.Any())
                throw new InvalidOperationException(
                    $"'{feature.Name}' xüsusiyy?ti {feature.Cars.Count} avtomobil? ba?l?d?r. " +
