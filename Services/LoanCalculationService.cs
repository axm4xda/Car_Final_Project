...
namespace Car_Project.Services
{
    public class LoanCalculationService : ILoanCalculationService
    {
        ...
        {
            if (carPrice <= 0)
                throw new ArgumentOutOfRangeException(nameof(carPrice), "Avtomobil qiym?ti 0-dan böyük olmal?d?r.");

            if (downPayment < 0 || downPayment >= carPrice)
                throw new ArgumentOutOfRangeException(nameof(downPayment), "?lkin öd?ni? 0 il? avtomobil qiym?ti aras?nda olmal?d?r.");

            if (annualInterestRate < 0)
                throw new ArgumentOutOfRangeException(nameof(annualInterestRate), "Faiz d?r?c?si m?nfi ola bilm?z.");

            if (loanTermMonths <= 0)
                throw new ArgumentOutOfRangeException(nameof(loanTermMonths), "Kredit müdd?ti 0-dan böyük olmal?d?r.");
            ...
            {
                // Annuitet (b?rab?r ayl?q öd?ni?) formulu:
                // M = P * [r(1+r)^n] / [(1+r)^n - 1]
